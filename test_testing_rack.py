...
from testing_rack import Config, RackError, StateStore
...
class StoreTest(unittest.TestCase):
  def setUp(self):
    ...
    config_path.write_text(json.dumps({
      "gateway_host": "gateway.invalid", "gateway_port": 22022, "default_lease_minutes": 60,
      "max_lease_minutes": 1440, "max_devices_per_reservation": 3,
      "devices": [
        {"name": f"NUT{i:03d}", "host": f"192.0.2.{i}", "model": "test device", "expected_ftdi_serial": f"FT{i}"}
        for i in range(1, 4)
      ]}))
  ...
  def test_capability_scope_and_release(self):
    ...
    self.assertEqual(self.store.resolve(result["capability"], "NUT001")["host"], "192.0.2.1")
    with self.assertRaises(RackError) as caught: self.store.resolve(result["capability"], "NUT003")
    self.assertEqual(caught.exception.status, 403)
    self.assertEqual(self.store.release(result["capability"])["released"], ["NUT001", "NUT002"])
    with self.assertRaises(RackError): self.store.current(result["capability"])
  ...
  def test_access_argument_can_never_be_parsed_as_an_ssh_option(self):
    ...
    self.assertTrue(argument.endswith("-NUT001")); self.assertFalse(argument.startswith("-"))
  ...
  def test_expiry_invalidates_capability_and_frees_devices(self):
    ...
    with self.assertRaises(RackError): self.store.resolve(result["capability"], "NUT001")

  def test_restart_preserves_active_reservation(self):
    result = self.reserve(2)
    restarted = StateStore(self.config, self.state, self.secret, self.clock)
    self.assertEqual(restarted.current(result["capability"])["devices"], result["devices"])
  ...
  def test_failed_atomic_commit_never_changes_memory_or_current_disk_state(self):
    ...
    def fail_final_replace(source, destination):
      if Path(destination) == self.state: raise OSError("injected final replace failure")
      return real_replace(source, destination)
    with mock.patch("testing_rack.os.replace", side_effect=fail_final_replace):
      with self.assertRaisesRegex(OSError, "injected"):
        self.reserve(1)
    self.assertEqual(self.store.state, before_memory)
    self.assertEqual(self.state.read_bytes(), before_disk)
    self.assertEqual(StateStore(self.config, self.state, self.secret, self.clock).state, before_memory)

  def test_concurrent_release_has_exactly_one_winner(self):
    result = self.reserve(1); barrier = threading.Barrier(8); outcomes = []
    def contender():
      barrier.wait()
      try: self.store.release(result["capability"]); outcomes.append("released")
      except RackError as exc: outcomes.append(exc.code)
  ...
  def test_final_device_race_has_one_winner(self):
    ...
    def contender(index):
      ...
      try:
        ...
        outcomes.append("won")
      except RackError as exc: outcomes.append(exc.status)
    threads = [threading.Thread(target=contender, args=(i,)) for i in range(8)]
    for thread in threads: thread.start()
    for thread in threads: thread.join()
    self.assertEqual(outcomes.count("won"), 1)
  ...
  def test_validation_boundaries(self):
    ...
    for args in cases:
      with self.subTest(args=args), self.assertRaises(RackError): self.store.reserve(*args)
    self.assertEqual(self.store.state["leases"], [])
  ...
  def test_init_command_creates_explicit_state(self):
    with tempfile.TemporaryDirectory() as directory:
      ...
      result = subprocess.run([
        sys.executable, str(Path(__file__).with_name("testing_rack.py")), "init",
        "--config", str(Path(__file__).with_name("config.json")),
        "--state", str(root / "state.json"), "--secret", str(root / "secret.key")
...
class ScaleTest(unittest.TestCase):
  def test_twenty_simultaneous_users_allocate_one_hundred_devices_without_overlap(self):
    with tempfile.TemporaryDirectory() as directory:
      root = Path(directory); config_path = root / "config.json"
      config_path.write_text(json.dumps({
        "gateway_host":"gateway.invalid", "gateway_port":22, "default_lease_minutes":60,
        "max_lease_minutes":1440, "max_devices_per_reservation":100,
        "devices":[{"name":f"NUT{i:03d}", "host":f"127.88.0.{i}", "model":"comma 4", "expected_ftdi_serial":f"NUT{i:03d}"} for i in range(1,101)]}))
      state, secret = root / "state.json", root / "secret"
      StateStore.initialize(state, secret); store = StateStore(Config.load(config_path), state, secret)
      barrier = threading.Barrier(20); results, failures = [], []
      def reserve_five(index):
        barrier.wait()
        try: results.append(store.reserve(f"user-{index:02d}", 5, 60, f"scale-user-key-{index:03d}"))
        except Exception as exc: failures.append(exc)
      threads = [threading.Thread(target=reserve_five, args=(i,)) for i in range(20)]
      for thread in threads: thread.start()
      for thread in threads: thread.join()
      self.assertEqual(failures, [])
      assigned = [device for result in results for device in result["devices"]]
      self.assertEqual(len(assigned), 100); self.assertEqual(len(set(assigned)), 100)
