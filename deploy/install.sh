...
install -o root -g root -m 0755 "$root/app.py" "$root/gateway.py" "$root/warm.py" /opt/testing-rack/
...
if [[ ! -s /etc/testing-rack/device_key ]]; then
...
if [[ ! -s /var/lib/testing-rack-gateway/.ssh/authorized_keys ]]; then
...
if [[ ! -s /var/lib/testing-rack-gateway/.ssh/known_hosts ]]; then
  echo "Set KNOWN_HOSTS to the verified comma host-key file." >&2
  exit 1
fi

if [[ ! -e /var/lib/testing-rack/state.json ]]; then
