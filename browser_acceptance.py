...
def run_case(browser, url: str, scenario: str, init_script: str, viewport: str) -> None:
  ...
  if init_script: context.add_init_script(init_script)
  ...
  try:
    ...
    page.locator("#count").select_option("1"); page.locator("#duration").select_option("60")
    started = time.monotonic(); page.locator("#reserve").click()
    page.locator("#reservation").wait_for(state="visible")
    command = page.locator("#command").inner_text()
    assert command.startswith("ssh rack@rack-gateway ") and command.endswith(tuple(f"-NUT{i:03d}" for i in range(1, 1000)))
    ...
    second = context.new_page(); second.goto(url, wait_until="networkidle")
    second.locator("#reservation").wait_for(state="visible")
    assert second.locator("#command").inner_text() == command
    second.close()
    page.once("dialog", lambda dialog: dialog.accept()); page.locator("#release").click()
    ...
    print(f"PASS chrome {scenario} {viewport} {int((time.monotonic()-started)*1000)}ms")
  finally:
    context.close()


def reserve(page, name: str) -> None:
  page.locator("#nickname").fill(name); page.locator("#count").select_option("1"); page.locator("#duration").select_option("60")
  page.locator("#reserve").click()


def release(page) -> None:
  page.once("dialog", lambda dialog: dialog.accept()); page.locator("#release").click(); page.locator("#reservation").wait_for(state="hidden")
...
def run_interleavings(browser, url: str) -> None:
  ...
  a.goto(url,wait_until="networkidle"); b.goto(url,wait_until="networkidle"); name="tabs-"+uuid.uuid4().hex[:8]
  for page in (a,b):
    page.locator("#nickname").fill(name); page.locator("#count").select_option("1"); page.locator("#duration").select_option("60")
  a.locator("#reserve").click(no_wait_after=True); b.locator("#reserve").click(no_wait_after=True)
  a.locator("#reservation").wait_for(state="visible",timeout=10000); b.locator("#reservation").wait_for(state="visible",timeout=10000)
  assert a.locator("#command").inner_text()==b.locator("#command").inner_text()
  release(a); b.locator("#reservation").wait_for(state="hidden",timeout=10000); context.close()

  # Separate profiles cannot create two leases with the same identity.
  first=browser.new_context(); second=browser.new_context(); p1=first.new_page(); p2=second.new_page()
  p1.goto(url,wait_until="networkidle"); p2.goto(url,wait_until="networkidle"); name="profiles-"+uuid.uuid4().hex[:8]
  reserve(p1,name); p1.locator("#reservation").wait_for(state="visible")
  reserve(p2,name); p2.locator("#message").filter(has_text="already has reservation").wait_for(state="visible")
  assert p2.locator("#reservation").is_hidden(); release(p1); first.close(); second.close()
  ...
  context=browser.new_context(permissions=[]); page=context.new_page(); page.goto(url,wait_until="networkidle")
  name="repeat-"+uuid.uuid4().hex[:8]; page.locator("#nickname").fill(name)
  page.evaluate("""() => { const form=document.querySelector('#reserve-form'); for(let i=0;i<8;i++) form.dispatchEvent(new Event('submit',{bubbles:true,cancelable:true})) }""")
  page.locator("#reservation").wait_for(state="visible")
  page.locator("#copy").click(); page.locator("#message").filter(has_text="Select and copy").wait_for(state="visible")
  assert page.locator("#reservation").is_visible(); release(page); context.close()
  ...
  with sync_playwright() as playwright:
    ...
    try:
      for scenario, script in SCENARIOS.items():
        for viewport in VIEWPORTS: run_case(browser, args.url, scenario, script, viewport)
